...
/**
 * Test class for Mark email as Deleted operation
 */
public class EmailMarkAsDeletedTest extends ConnectorIntegrationTestBase {
    ...
    @Test(enabled = true, groups = {"wso2.ei"}, description = "Test marking email as deleted")
    public void testMarkAsDeleted() throws Exception {
        ...
        assertEquals(response.getData(), "{\"result\":{\"success\":true}}",
                "Email is not marked as deleted.");
        MimeMessage[] newMessages = GreenMailServer.getInstance().getReceivedMessages();
        assertTrue(newMessages[0].getFlags().contains(Flags.Flag.DELETED));
    }

    @Test(enabled = true, groups = {"wso2.ei"}, description = "Test marking email as deleted without specifying folder")
    ...
    @Test(enabled = true, groups = {"wso2.ei"}, description = "Test marking email as deleted without Email ID")
    public void testMarkAsDeletedWithoutEmailID() throws Exception {

        GreenMailServer.getInstance().clear();
        logViewerClient.clearLogs();
        EmailTestUtils.sendSampleEmail(EmailTestUtils.Constants.PROTOCOL_IMAP);
        sendJsonRestRequest(getProxyServiceURLHttp("MarkAsDeleted"), "POST",
